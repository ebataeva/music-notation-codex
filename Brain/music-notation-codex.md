...
- `.planning/REQUIREMENTS.md`, `.planning/ROADMAP.md` — being regenerated by `/gsd-new-project` (old single-phase versions are stale)
- `scripts/generate_cello_dark_ostinato.py`, `scripts/harmony_advisor.py` — existing engine to refactor into `core/`

## Next implementation phase

Roadmap is being (re)generated. Build order starts at Phase 1: core library skeleton + validators (`validate_pitch`, `validate_bar_duration`, dataclasses, merged `MoodPreset`) — pure Python, no Streamlit yet. Then `/gsd-plan-phase 1`.
